...
Backend-first learning project. Core is a Flask app wrapping the receipt-processing
...
Flask app with one route, `POST /receipts`, accepting an image and returning JSON.
...
time, price-per-item history. Can live behind another Flask endpoint returning
...
- A loose JSON schema for a "line item" now, so Phases 1–3 all target the same
  shape.
## Status
 
Not yet started. Next step under discussion: scaffold Phase 0 (Flask skeleton +
mock endpoint + basic test), or talk through OCR-approach tradeoffs first.